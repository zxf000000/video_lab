  ...
  return (
    <header className="sticky top-5 z-10 rounded-[20px] border border-line bg-panel/80 px-4 py-3 shadow-glow backdrop-blur-xl">
      <div className="flex flex-wrap items-center gap-3">
        <Link href={backHref} className="inline-flex shrink-0 items-center gap-2 rounded-full bg-panel2 px-3 py-1.5 text-xs font-medium text-gray-400 transition hover:text-gray-100">
          ...
          <div className="relative shrink-0" ref={menuRef}>
            ...
            {menuOpen ? (
              <div className="absolute right-0 top-full z-20 mt-2 w-56 overflow-hidden rounded-2xl border border-line bg-panel shadow-lg">
                <button
                  className="flex w-full items-center gap-3 px-4 py-3 text-left text-sm text-gray-300 transition hover:bg-panel2"
                  onClick={() => handleRegenerate(false)}
                >
                    <IconRefresh size={14} stroke={2} className="text-gray-500" />
                  <div>
                    <div className="font-medium">重新生成全部</div>
                    <div className="mt-0.5 text-xs text-gray-500">重新生成项目大纲和角色卡</div>
                  </div>
                </button>
                <button
                  className="flex w-full items-center gap-3 border-t border-line px-4 py-3 text-left text-sm text-gray-300 transition hover:bg-panel2"
                  onClick={() => handleRegenerate(true)}
                >
                  <IconRefresh size={14} stroke={2} className="text-mint" />
                  <div>
                    <div className="font-medium">保留大纲，重新生成角色卡</div>
                    <div className="mt-0.5 text-xs text-gray-500">保留当前大纲，只重新提取角色卡</div>
                  </div>
                </button>
              </div>
            ) : null}
        ...
        <ApiBaseBadge className="hidden shrink-0 rounded-full border border-line bg-panel2 px-3 py-1.5 text-[11px] font-medium text-gray-500 sm:inline" />
      </div>
