...
import { generateBeats, updateBeats, getBeatsVersions, restoreBeatsVersion } from "../../api";
...
export default function BeatsTab({ project, isPending, onRunAction }: any) {
  const [editing, setEditing] = useState(false);
  const [draftCn, setDraftCn] = useState(project.beats_content || "");
  const [draftEn, setDraftEn] = useState(project.beats_content_en || "");
  ...
  const beatsTasks = project.tasks?.filter(
    (t: any) => t.task_type === "generate_beats" || t.task_type === "regenerate_from_stage"
  );

  const content = lang === "cn" ? project.beats_content : project.beats_content_en;
  const draft = lang === "cn" ? draftCn : draftEn;
  const setDraft = lang === "cn" ? setDraftCn : setDraftEn;

  function startEditing() {
    setDraftCn(project.beats_content || "");
    setDraftEn(project.beats_content_en || "");
    setEditing(true);
  }
  ...
  async function saveBeats() {
    setSaving(true);
    try {
      await updateBeats(project.id, draftCn, draftEn);
      setEditing(false);
  ...
  async function handleGenerate() {
    setGenerating(true);
    try {
      await generateBeats(project.id);
      await onRunAction(() => Promise.resolve());
    } catch (err: any) {
  ...
  async function loadVersions() {
    ...
    try {
      const data = await getBeatsVersions(project.id);
  ...
  async function handleRestore(versionId: any) {
    ...
    try {
      await restoreBeatsVersion(project.id, versionId);
      setVersions(null);
      await onRunAction(() => Promise.resolve());
    } finally {
      setRestoringId(null);
    ...
    <div className="flex flex-col gap-6">
      <section className="rounded-[28px] border border-line bg-panel p-6 shadow-glow">
        <div className="flex items-center justify-between">
          <h3 className="text-xl font-semibold text-slate-900">节拍列表</h3>
          <div className="flex flex-wrap gap-3">
            ...
            {editing ? (
              <>
                <ActionButton icon={IconDeviceFloppy} disabled={saving} label="保存" onClick={saveBeats} />
        ...
        {content ? (
          ...
          <EmptyState text={project.screenplay_content || project.story_content ? "尚未生成节拍，点击「AI 生成」开始。" : "请先生成剧本化文本或剧情内容。"} />
        )}
      </section>
      ...
      {beatsTasks?.length ? <TaskPanel tasks={beatsTasks} /> : null}
      <RefineDrawer
        open={refineOpen}
        ...
        onApply={(newContent: string) => {
          ...
          updateBeats(project.id, lang === "cn" ? newContent : draftCn, lang === "en" ? newContent : draftEn).then(() => {
